...
alter table employee drop column emp_state;
...
select *,
rank() over(partition by d.dept_name order by e.salary desc ) as emp_rank
from employee as e
join
department as d
on e.dept_id=d.dept_id;
...
select *,
rank() over(order by e.salary desc) as emp_rank
from employee as e
join
department as d
on e.dept_id=d.dept_id;
